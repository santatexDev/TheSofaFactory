(function($) {
  ...
  StyleHatch.Product = (function() {
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      _updateStoreAvailabilityContent: function(evt) {
        if (this.storeAvailability && evt.variant) {
          this.storeAvailability.updateContent(
            evt.variant.id,
            this.productSingleObject.title
          );
  ...
  StyleHatch.AjaxCart = (function() {
    ...
    function showCartPreview(productData, cartData) {
      ...
      var productSellingPlan = productData.selling_plan_allocation
        ? productData.selling_plan_allocation.selling_plan.name
        : null;

      if (productSellingPlan) {
        var $productSellingPlan = $cartPreview.find('.product-selling-plan').empty();
        $productSellingPlan.append(productSellingPlan);
      }

      
      var productProperties = productData.properties;
      if (productProperties) {
        if (Object.keys(productProperties).length > 0) {
          var $productOptions = $cartPreview.find('.product-options').empty();
          for (const property in productProperties) {
            var key = property;
            var value = productProperties[property];
            if (!key.startsWith('_') && value != '') {
              $productOptions.append(`<li><strong>${key}:</strong> ${value}</li>`);
            }
          }
        }
      }
